...
export async function POST(req: Request) {
  try {
    ...
    const response = await claimKy.post("api/staking/deduct", {
    json: body,
  });
  ...
  //   const body = { walletAddress, amount };
  //   const headers = await buildStakingHeaders(apiKey, apiSecret, body);

  //   const res = await fetch(
  //     "https://jungl-api.mybitverse.com/api/staking/deduct",
  //     {
  //       method: "POST",
  //       headers: {
  ...
  //   if (!res.ok) {
  //     return NextResponse.json(
  //       { error: "Deduct failed", details: data },
  //       { status: res.status }
  //     );
  //   }
